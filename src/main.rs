...
fn main() {
    ...
    amb.add_custom_resource(Rc::new(RefCell::new(DBResource::new(data_db, scopes_db, env))));
    
    // Example 1: Write to confederation scope
    println!("=== Test 1: Writing to /conf/greeting ===");
    let _write_result = smx::eval("_ @{DB, IO} = (\"/conf/greeting\", \"hello from confederation\"): DB.write", &mut amb);
    println!("Write completed");

    // Example 2: Read from confederation scope
    println!("\n=== Test 2: Reading from /conf/greeting ===");
    let read_result = smx::eval("_ @{DB, IO} = \"/conf/greeting\": DB.read: IO.print", &mut amb);
    println!("Read result: {:?}", read_result);

    // Example 3: Write to commune scope
    println!("\n=== Test 3: Writing to /commune/cypherpunx/laws/001 ===");
    let _write_result2 = smx::eval("_ @{DB, IO} = (\"/commune/cypherpunx/laws/001\", \"law: universal suffrage\"): DB.write", &mut amb);
    println!("Write completed");

    // Example 4: Read from commune scope
    println!("\n=== Test 4: Reading from /commune/cypherpunx/laws/001 ===");
    let read_result2 = smx::eval("_ @{DB, IO} = \"/commune/cypherpunx/laws/001\": DB.read: IO.print", &mut amb);
